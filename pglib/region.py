...
class Region(object):
    ...
    def split(self, min_leaf_size):
        ...
        split_h = random.uniform(0, 1) > 0.5
        if self.width > self.height and self.width / self.height >= 1.25:
            split_h = False
        elif self.height > self.width and self.height / self.width >= 1.25:
            split_h = True

        max_d = (self.height if split_h else self.width) - min_leaf_size # determine the maximum height or width
        if max_d <= min_leaf_size:
            return False # the area is too small to split any more...

        split = random.randint(min_leaf_size, max_d) # determine where we're going to split

        # Create our left and right children based on the direction of the split
        if split_h:
            self.left_child = Region(self.x1, self.y1, self.x2, self.y1 + split)
            self.right_child = Region(self.x1, self.y1 + split, self.x2, self.y2)
        else:
            self.left_child = Region(self.x1, self.y1, self.x1 + split, self.y2)
            self.right_child = Region(self.x1 + split, self.y1, self.x2, self.y2)
    ...
    def is_safe(self, index, visited):
        i, j = index
    ...
    def DFS(self, index, visited, island):
        ...
        for k in range(4):
            if self.is_safe((i + row_nbr[k], j + col_nbr[k]), visited):
                self.DFS((i + row_nbr[k], j + col_nbr[k]), visited, island)
    ...
    def get_remaining_area(self, region, direction):
        """
